...
class MainProgram{
	...
	void DriverMenu(Driver CurretDriver){
		printf("\n");
        printf(" *************************\n");
		...
		string reward = CurretDriver.GetReward();
		string punishment = CurretDriver.GetPunishment();

		scanf("%d", &input);
		switch(input){
			case 1:
				CurretDriver.GetReward();
			case 2:
				CurretDriver.GetPunishment();
			case 3:
				MainMenu();
		}
	}
